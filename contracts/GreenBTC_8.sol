    ...
    ERC721Upgradeable
{
    ...
    function _mintNFT(Green_BTC calldata gbtc) internal {
        ...
        require(_data[gbtc.height].cellCount == 0, "GBTC: only grey block can be mint");
    ...
    {
        require(_data[gbtc.height].cellCount == 0, "GBTC: only grey block can be mint");
        require((gbtc.cellCount * (10**9)) <= amountART, "GBTC: not enough ART");  //one ART per cell
        //verify signature
        _authVerify(gbtc, sig);
    ...
    function setImageContractAddress(address newImageContractAddress) public onlyManager {
        ...
        _imageContract = newImageContractAddress;
    }

    function approveBuilder(address[] calldata tokens) public onlyManager {
        require(_arkreenBuilder != address(0), "GBTC: No Builder");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory){
        ...
        require(_data[tokenId].height != 0, "GBTC: no such token minted");
